...
const result = await runTauriE2EBenchmark({
  dataDirectory: dirname(catalogPath),
  flow: "catalog",
  iterations,
});
...
console.log(
  `[bench:e2e:catalog] n=${iterations} p50=${p50}ms p95=${p95}ms max=${result.max}ms threshold=${threshold}ms -> ${pass ? "PASS" : "FAIL"}`,
);
...
if (updateBaseline && pass) {
  ...
  if (baselineMs !== baseline.baselineMs) {
    ...
    console.log(`[bench:e2e:catalog] baseline updated: baselineMs=${baselineMs}`);
  }
}
